...
const QualityMonitoring = () => {
  const [monitorsDrift, setMonitorsDrift] = useState<MonitorsDrift | null>(
    null
  );
  const [monitorsModelDrift, setMonitorsModelDrift] =
    useState<MonitorsModelDrift | null>(null);
  ...
  const missingValuesData =
    monitorsDrift?.top_missing_values.slice(0, 5).map((item) => ({
      name: item.engine_id,
      count: item.count,
    })) || [];

  const outliersData =
    monitorsDrift?.top_outliers.slice(0, 5).map((item) => ({
      name: item.engine_id,
      count: item.count,
    })) || [];
  ...
  const dataDriftData =
    monitorsDrift?.top_data_drift.slice(0, 5).map((item) => ({
      name: item.engine_id,
      count: item.count,
    })) || [];
  ...
  const systemHealthData = monitorsDrift
    ? [
        {
          name: "Healthy",
          value: monitorsDrift.data_quality.healthy_engines,
          fill: "#1eb3e1ff",
        },
        {
          name: "Medium ",
          value: monitorsDrift.data_quality.medium_issue_engines,
          fill: "hsl(var(--warning))",
        },
        {
          name: "Critical",
          value: monitorsDrift.data_quality.critical_issue_engines,
  ...
  const criticalIssues =
    monitorsDrift?.critical_issues.sort((a, b) => {
  ...
  const modelStability = monitorsModelDrift?.total_engines
    ? `${((stableEngines / monitorsModelDrift.total_engines) * 100).toFixed(
        0
      )}%`
    : "0%";
  ...
  const criticalIssuesColumns: TableColumn<Record<string, unknown>>[] = [
    ...
    },
    {
      ...
      render: (value) => (
        <Badge
          variant={getSeverityVariant(value as string)}
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <MetricCard
          ...
          subtitle={`${stableEngines}/${
            monitorsModelDrift?.total_engines || 0
          } engines stable`}
        ...
        <Card>
          ...
          <CardContent>
            {outliersData.length === 0 ? (
              ...
              <BarChart
                data={outliersData}
                bars={[{ dataKey: "count", fill: "#f59e0b", name: "Outliers" }]}
                height={250}
              />
            )}
          </CardContent>
